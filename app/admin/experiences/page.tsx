...
export default function ExperiencesAdmin() {
  const { data: experiences, mutate } = useSWR<Experience[]>("/api/experiences", fetcher)
    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Gerez vos experiences professionnelles</p>
        </div>
        <button
          onClick={startCreate}
      ...
      {(creating || editing) && (
        <div className="mb-8 rounded-2xl border border-primary/30 bg-card/50 p-6 backdrop-blur-sm">
          <h3 className="mb-4 font-serif text-lg font-bold text-foreground">
            {creating ? "Nouvelle experience" : "Modifier l'experience"}
          </h3>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Entreprise</label>
              <input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="w-full rounded-lg border border-border bg-input px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none" />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })} placeholder="Juin 2025" className="w-full rounded-lg border border-border bg-input px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none" />
            </div>
            <div>
      ...
      <div className="space-y-4">
        {experiences?.map((exp) => (
          <div key={exp.id} className="group flex items-start justify-between rounded-xl border border-border bg-card/30 p-5 backdrop-blur-sm transition-all hover:border-primary/20">
            <div className="flex-1">
              <div className="flex items-center gap-3">
                <h3 className="text-base font-semibold text-foreground">{exp.title}</h3>
                <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">#{exp.display_order}</span>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">{exp.company} - {exp.location}</p>
              <p className="mt-0.5 font-mono text-xs text-primary">{exp.start_date} - {exp.end_date || "Present"}</p>
            ...
            <div className="flex gap-2 opacity-0 transition-opacity group-hover:opacity-100">
              <button onClick={() => startEdit(exp)} className="rounded-lg p-2 text-muted-foreground hover:bg-primary/10 hover:text-primary" aria-label="Modifier">
                <Pencil className="h-4 w-4" />
              </button>
              <button onClick={() => handleDelete(exp.id)} className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive-foreground" aria-label="Supprimer">
